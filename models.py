...
class LSTMClassifierModel(BaseModel):
    ...
    def __init__(
        ...
        super().__init__(model_name, lr, weight_decay, loss_func, warmup_steps, total_steps, LDAM_start, lr_scheduler)
        self.save_hyperparameters()
        self.LSTM = nn.Sequential(nn.LSTM(1024, 1024, dropout=0.2, batch_first=True, bidirectional=False))
        self.classifier = nn.Sequential(
            nn.Dropout(p=0.2),
            nn.Linear(1024, 512),
            nn.GELU(),
    ...
    def forward(self, input_ids=None, ss=None, os=None, **kwargs):
        ...
        lstm_input = torch.cat((pooled_output.unsqueeze(1), seq_output), dim=1)
        lstm_output, (h_n, c_n) = self.LSTM(lstm_input)
        n_h_n = h_n.transpose(0, 1).contiguous().view(h_n.shape[1], h_n.shape[0] * h_n.shape[2])
        logits = self.classifier(n_h_n)
